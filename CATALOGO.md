...
| Medir jitter separándolo del movimiento real del operador (residuo de alta frecuencia vs. promedio móvil) | webar | `src/core/ar/JitterProbe.ts` | Aplica a cualquier medición de estabilidad donde quien mide también se mueve. La varianza cruda **no** sirve |
| Hit-test de toque sobre objeto 3D midiendo *a cuántos píxeles del objeto cayó el fallo* | webar | `src/core/ar/ScreenRaycaster.ts` | Convierte "la interacción se siente poco confiable" en "hacer el área de toque de N px". Un booleano hit/miss no puede |
| Generar GLB animados por código, a tamaños controlados, sin dependencias | webar | `tools/make-character.mjs` + `check-glb.mjs` | Útil cuando el *tamaño del asset* es la variable a medir, o para evitar licencias de modelos ajenos |
...
| MindAR + A-Frame | GLB animado anclado al target, con jitter y costo de asset medidos | webar | Implementado, pendiente de prueba física |
| MindAR + A-Frame | Toque/raycasting sobre el objeto anclado, con latencia y falsos positivos | webar | Implementado, pendiente de prueba física |
| three.js | Reproducción y cambio de clips sin aframe-extras (`AnimationMixer` directo) | webar | Implementado, pendiente de prueba física |
